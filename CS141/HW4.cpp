...
void Record::show_all(){
  ...
  for (int i=0; i<3; i++){     //print all three inventory counts of item in the three differ\
ent locations
  cout.width(11);
  cout<<count[i];
  }
...
void warn(vector<Record> &v);  //checks number of items in all records and displays what is h\
as less than 10 items left at a specific location
...
void warn(vector<Record> &v){
  ...
  for(int recordNum=0; recordNum<(v.size()); recordNum++){  //first need to go through each v\
ector element
    for(int placeNum=0; placeNum<3; placeNum++){ //need to check every place in each record, \
3 here because there are 3 places

      if (v[recordNum].count[placeNum]<min){
        cout<<"Warning: "<<v[recordNum].name<<" | "<<places[placeNum]<<": "<<v[recordNum].cou\
nt[placeNum]<<endl;
      }
