...
int main(){
  ...
  while(in.get(advice)){ //if at the end of the file and no more char to read, it wil\
l be false
    cout<<advice;
  }
  ...
  while(count<2){   //end after two '\n'
    ...
    if(advice=='\n') //count first and when back to back
      count++;
    else if(advice!='\n'&&count==1) //only subtract when count once already so it is \
never negative or less than zero
      count--;
  }
